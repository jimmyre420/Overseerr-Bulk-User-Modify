<#
.SYNOPSIS
	Enables specific email notifications for all Overseerr users with CORRECTED bitmask values.

.DESCRIPTION
	This script connects to the Overseerr API and configures email notification settings
	for all users. It uses reverse-engineered bitmask values to ensure "Approved",
	"Declined", and "Available" are correctly mapped for standard users.
	
	The script fixes the bitmask mapping issues found in v0.07.

.NOTES
	File Name      : Overseerr_Bulk-User-Email-Edit_v0.08.ps1
	Author         : Modified by Antigravity AI Assistant
	Prerequisite   : PowerShell 5.1 or later, network access to Overseerr instance
	Version        : 0.08
	Date           : 2026-01-23 03:35
	
.CHANGELOG
	v0.08 (2026-01-23 03:35)
	- Updated Bitmask values based on live API checking of Standard User (ID 6):
	  - Approved: 4
	  - Available: 8
	  - Declined: 64
	- Updated default mask to 76 (4+8+64).
	- Updated Legend.
	
	v0.07 (2026-01-23 02:04)
	- Fixed '400 Bad Request' using specific settings endpoint.
#>
...
# REVERSE-ENGINEERED Bitmask values (Verified on User 6):
[int]$BIT_APPROVED = 4     # Request Approved
[int]$BIT_AVAILABLE = 8     # Request Available
[int]$BIT_DECLINED = 64    # Request Declined
...
try {
	...
	Write-Host "# Version 0.08" -ForegroundColor Cyan
	Write-Host "########################################" -ForegroundColor Cyan
	Write-Host ""
	...
	[int]$finalMask = 0
	[bool]$useDefaults = Get-UserChoice -Prompt "Use default notification settings (4:Approved, 64:Declined, 8:Available)?" -DefaultChoice "Y" -TimeoutSeconds 10
	...
	if ($useDefaults) {
		...
		if (Get-UserChoice -Prompt "  - Enable Bit 4  (Request Approved)?" -DefaultChoice "Y") { $finalMask = $finalMask -bor $BIT_APPROVED }
		if (Get-UserChoice -Prompt "  - Enable Bit 64 (Request Declined)?" -DefaultChoice "Y") { $finalMask = $finalMask -bor $BIT_DECLINED }
		if (Get-UserChoice -Prompt "  - Enable Bit 8  (Request Available)?" -DefaultChoice "Y") { $finalMask = $finalMask -bor $BIT_AVAILABLE }
	...
	Write-Host "########################################" -ForegroundColor Cyan
	Write-Host ""
	Write-Host "Notification Mask Legend (Verified User 6):" -ForegroundColor Yellow
	Write-Host "  Bit 4   : Request Approved"
	Write-Host "  Bit 8   : Request Available"
	Write-Host "  Bit 64  : Request Declined"
	Write-Host "  ------------------------------------"
	Write-Host "  Total Mask = Sum of enabled bits (e.g., 76 = 4+8+64)"
	Write-Host ""
