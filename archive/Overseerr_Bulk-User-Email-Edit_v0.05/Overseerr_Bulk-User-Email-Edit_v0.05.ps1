<#
.SYNOPSIS
	Enables specific email notifications for all Overseerr users with interactive selection.
...
.DESCRIPTION
	...
	to enable (Request Approved, Request Declined, Request Available) or use defaults.
	
	The script provides a detailed summary report at the end showing the assigned
	permissions for each user.
	
	Test mode allows you to preview changes without actually modifying user settings.

.NOTES
	File Name      : Overseerr_Bulk-User-Email-Edit_v0.05.ps1
	Author         : Modified by Antigravity AI Assistant
	Prerequisite   : PowerShell 5.1 or later, network access to Overseerr instance
	Version        : 0.05
	Date           : 2026-01-23 01:48
	
.CHANGELOG
	v0.05 (2026-01-23 01:48)
	- Added interactive notification selection logic.
	- Set Verbose mode default to 'Y' per user request.
	- Added detailed post-execution summary table with permission counts.
	- Optimized results collection for summary reporting.
	
	v0.04 (2026-01-23 01:42)
	- Fixed character corruption and ensured 100% Tab indentation.
#>
...
try {
    ...
    Write-Host "# Version 0.05" -ForegroundColor Cyan
    Write-Host "########################################" -ForegroundColor Cyan
    Write-Host ""
    ...
    # INTERACTIVE PROMPT: Verbose Mode (Default: Y per user request)
